...
# eulerian circuit: hierholzer's

Eulerian Circuit: closed path in a graph traversing every edge once, starting and ending on same vertex

* Every vertex even number of degrees (edges connected to it)
* Eulerian path doesn't need to start and end on same vertex
    ...
    vector<vector<int>> validArrangement(vector<vector<int>>& pairs) {
        ...
        while (!stack.empty()) {
            ...
            if (!adj[node].empty()) {
                ...
                // remove edges while traversing graph
                adj[node].pop_back();
